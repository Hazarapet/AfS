...
def DenseNet(nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.0, dropout_rate=0.0, classes=1000, weights_path=None):
...
def load_weights(_model, weights_path):
    ...
    for i, layer in enumerate(_model.layers):
        ...
        if layer.name in weights:
            w = weights[layer.name]
            ar = []
            if np.array(layer.get_weights()).ndim == 5:
                wt = w.values()[0][()]
                wt = wt.transpose((2, 3, 1, 0))
                ar.append(np.array(wt))
            else:
                for v in w.values():
